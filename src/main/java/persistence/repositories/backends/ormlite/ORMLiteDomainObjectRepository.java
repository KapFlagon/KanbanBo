package persistence.repositories.backends.ormlite;
...
public abstract class ORMLiteDomainObjectRepository<T, U> implements Repository<T, U> {
    ...
    protected T getObjectFromDatabaseByID(U u) throws SQLException {
        T t = dao.queryForId(u);
        if (u != null) {
            repositoryList.add(t);
            return t;
    ...
    public T getByID(U u) throws SQLException, IOException {
        T t  = getObjectFromListById(u);
        if(t == null) {
            return getObjectFromDatabaseByID(u);
        } else {
            return t;
        }
    }

    protected abstract T getObjectFromListById(U u);
